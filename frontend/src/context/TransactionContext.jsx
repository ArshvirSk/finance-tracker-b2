...
export const TransactionProvider = ({ children }) => {
  ...
  useEffect(() => {
    fetch("https://finance-tracker-b1-service.onrender.com/transactions")
      .then((res) => res.json())
      .then((data) => {
  ...
  const addTransaction = async (tx) => {
    try {
      const res = await fetch(
        "https://finance-tracker-b1-service.onrender.com/transactions",
        {
          method: "POST",
  ...
  const deleteTransaction = async (_id) => {
    try {
      await fetch(
        `https://finance-tracker-b1-service.onrender.com/transactions/${_id}`,
        {
          method: "DELETE",
        }
      );
